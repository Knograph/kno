...
- **A keyless [cosign](https://docs.sigstore.dev/) signature** over `checksums.txt`
  (`checksums.txt.sig` + `checksums.txt.pem`). Keyless means there is no private key in existence,
  so there is no key material to steal, rotate, or leak — the signing identity is the release
  workflow's OIDC token, recorded in the public Rekor transparency log.
