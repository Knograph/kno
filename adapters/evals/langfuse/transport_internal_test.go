...
func TestBasicAuthValue(t *testing.T) {
	t.Parallel()
	// The documented Langfuse scheme: public key as user, secret key as
	// password, base64-encoded — base64 is not encryption, which is exactly
	// why the endpoint checks refuse to send it over plain HTTP.
	want := "Basic " + base64.StdEncoding.EncodeToString([]byte("pk-1:sk-1"))
	if got := basicAuthValue("pk-1", "sk-1"); got != want {
...
func TestRedact(t *testing.T) {
	t.Parallel()
	// The credential is the basic-auth pair: the secret key itself and the
	// full Authorization token (the token embeds the secret half). Both must
	// disappear from every message. The token is computed, never spelled out:
	// a literal base64 blob in source trips the secrets scan.
	token := "Basic " + base64.StdEncoding.EncodeToString([]byte("test-secret-key:"))
	e := &Evals{basic: token, secretKey: "test-secret-key"}
	msg := "request failed with test-secret-key and again " + token
	got := e.redact(msg)
	for _, frag := range []string{"test-secret-key", token} {
...
func TestPageReaderHandlesAnyKeyOrder(t *testing.T) {
	t.Parallel()
	// meta before data: the envelope walk must not assume order.
	body := `{"meta":{"page":1,"limit":100,"totalItems":1,"totalPages":1},` +
		`"data":[{"id":"it-1","input":{"question":"q"}}]}`
	...
	if !bytes.Contains(raw, []byte(`"it-1"`)) {
		t.Errorf("row = %s, want the item object", raw)
	}
	more, err := pr.morePages()
	if err != nil {
		t.Fatalf("morePages: %v", err)
	}
	if more {
		t.Error("a one-page envelope must report no more pages")
	}
	if _, ok, err := pr.nextRow(); err != nil || ok {
		t.Errorf("nextRow after the last item = %v,%v, want EOF", ok, err)
	}
}
...
func TestPageReaderIgnoresUnknownFields(t *testing.T) {
	...
	body := `{"data":[{"id":"it-1","input":{"question":"q"}}],` +
		`"something_new":{"x":1},"meta":{"page":1,"limit":100,"totalItems":1,"totalPages":1}}`
	pr, err := newPageReader(bytes.NewReader([]byte(body)))
	if err != nil {
		t.Fatalf("newPageReader: %v", err)
	}
...
func TestEndpointCheckCopiesTheDestinationPrecedent(t *testing.T) {
	...
	// These cases mirror adapters/evals/langsmith/endpoint.go's behavior,
	// which itself mirrored adapters/agent/internal/transport/destination.go;
	// the adapter must not drift from the precedent it copied.
	tests := []struct {
		raw      string
		insecure bool
		private  bool
		...
		{raw: "https://cloud.langfuse.com", wantErr: ""},
		{raw: "http://cloud.langfuse.com", insecure: true, wantErr: ""},
...
func TestCheckResolvedAppliesThePolicyToTheResolvedAddress(t *testing.T) {
	...
	// These mirror the langsmith adapter's checkResolved: the address here is
	// what the resolver produced, so "localhost" appears as its literal IP.
	tests := []struct {
		addr    string
		private bool
		wantErr string
...
func TestRowCapBindsTheReadNotJustTheRow(t *testing.T) {
	...
	body := `{"data":[` + big + `,{"id":"after"},{"id":"also-after"}],"meta":{"page":1}}`
