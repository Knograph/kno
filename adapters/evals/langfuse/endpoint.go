...
// made.
//
// The API keys travel to this host on every request — as basic auth, which
// is base64, not encryption — so the host itself must be one the keys are
// allowed to reach. These rules are copied verbatim from
// adapters/evals/langsmith/endpoint.go, which itself copied them from
// adapters/agent/internal/transport's Destination check (destination.go) —
// the same policy the agent adapters enforce for the same reason. They are
...
func checkEndpoint(raw string, allowInsecureBaseURL, allowPrivateAddress bool) error {
	...
	if err != nil {
		...
		// it back, twice.
		return fmt.Errorf("langfuse: the endpoint could not be parsed")
	}
	...
	if u.User != nil {
		return fmt.Errorf("langfuse: the endpoint carries a credential in its userinfo; " +
			"remove it and set LANGFUSE_PUBLIC_KEY and LANGFUSE_SECRET_KEY instead")
	}
	...
	case "http":
		if !allowInsecureBaseURL {
			return fmt.Errorf("langfuse: %s is plain HTTP, which sends the request and the "+
				"API keys in the clear (basic auth is base64, not encryption); set "+
				"LANGFUSE_HOST to an https URL, or pass --allow-insecure-base-url "+
				"to accept the risk", u.Host)
		}
	default:
		return fmt.Errorf("langfuse: scheme %q is not http or https", u.Scheme)
	}

	if u.Host == "" {
		return fmt.Errorf("langfuse: the endpoint %q has no host", raw)
	}
	return checkAddress(u.Hostname(), allowPrivateAddress)
}

// checkAddress applies the private-address rules. Copied verbatim from
// adapters/evals/langsmith/endpoint.go (thence transport/destination.go's
// checkAddress), same rationale line for line.
//
...
func checkAddress(hostname string, allowPrivateAddress bool) error {
	...
	if ip == nil {
		...
		if !plausibleDNSName(hostname) {
			return fmt.Errorf("langfuse: %q is neither a hostname nor a canonical IP "+
	...
	if ip.IsLinkLocalUnicast() || ip.IsLinkLocalMulticast() {
		return fmt.Errorf("langfuse: %s is link-local, which is where cloud instance "+
	...
	if ip.IsLoopback() || ip.IsPrivate() || ip.IsUnspecified() {
		return fmt.Errorf("langfuse: %s is a private address. A self-hosted Langfuse "+
			"deployment is a real use, so this is opt-in — but the same request shape "+
			"reaches an internal service, and the API keys would travel to it", ip)
	}
	return nil
}
...
// adapters/evals/langsmith/endpoint.go, same rationale line for line.
//
...
// adapters/evals/langsmith/endpoint.go (thence transport/destination.go).
//
