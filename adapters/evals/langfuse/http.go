...
// basicAuthValue is the Authorization header value for Langfuse's documented
// auth: public key as user, secret key as password, base64-encoded.
...
// is always the page timeout, because a page bigger or slower than the caps
// is broken, not slow. Same contract as adapters/evals/langsmith's
// Options.HTTPClient and the agent adapters'.
//
// The private-address policy is installed as a dial-time recheck of the
// RESOLVED address, mirroring the langsmith adapter — the config-time check
// in checkEndpoint cannot see where a hostname resolves. A transport the
// caller supplied that is not an *http.Transport (a test stub) has no dialer
// to reach, so the config-time check is what applies to it.
...
// redirect but NOT a same-host one — and this credential is the pair of API
// keys, not a per-host header. Refusing all redirects — same host or not —
// is the only safe policy, and the datasets and dataset-items endpoints have
// no legitimate reason to redirect.
func refuseRedirect(req *http.Request, _ []*http.Request) error {
	return fmt.Errorf("langfuse: refusing a redirect to %s; the credentials must not follow one", req.URL.Host)
}
...
//
// No error ever contains a key or a response body.
func (e *Evals) do(ctx context.Context, path string, query url.Values) (*http.Response, error) {
	...
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		...
		if err != nil {
			return nil, fmt.Errorf("langfuse: building the request: %s", e.redact(err.Error()))
		}
		req.Header.Set("Authorization", e.basic)
		req.Header.Set("Accept", "application/json")

		resp, err := e.transport.Do(req)
		if err != nil {
			// The transport error quotes the request URL; the keys are not in
			// it, but redact defensively so a future change to the URL cannot
			// leak the credential.
			lastErr = fmt.Errorf("langfuse: request failed: %s", e.redact(err.Error()))
			continue
		}
		...
		if attempt == maxAttempts {
			lastErr = fmt.Errorf("langfuse: the dataset API kept answering 429 after %d "+
				"attempts; it is throttling this address", maxAttempts)
			continue
		}
		lastErr = fmt.Errorf("langfuse: the dataset API answered 429; retrying in %s", wait)
...
// redact removes every occurrence of the credential from a message.
//
// The credential is the basic-auth pair: the secret key itself and the full
// Authorization header value (the public key half is not secret, but the
// token contains the secret half). Errors must never carry either — they
// would land in a log line, a trace, or a report. Applied at every
// error-construction point rather than at the package boundary, so nothing
// can slip through.
func (e *Evals) redact(msg string) string {
	if e.basic != "" {
		msg = strings.ReplaceAll(msg, e.basic, "[redacted]")
	}
	return strings.ReplaceAll(msg, e.secretKey, "[redacted]")
}

// pageLimitError reports the pagination backstop once more than maxPages
// pages have been fetched. A server whose meta never advances — or that
// serves more pages than its own totalPages promised — is caught here rather
// than spun on forever.
func pageLimitError(fetched int) error {
	if fetched > maxPages {
		return fmt.Errorf("langfuse: pagination exceeded %d pages; the dataset keeps "+
			"issuing fresh pages, so the fetch stops here rather than spin", maxPages)
	}
	return nil
}
