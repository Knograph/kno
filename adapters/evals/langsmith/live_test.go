...
// liveEvals builds an Evals from the environment, skipping when the key or
// dataset is absent. Endpoint defaults to the hosted API; a self-hosted
// deployment is selected by LANGSMITH_ENDPOINT and must be opted into with
// the same flags the CLI uses, so the live posture matches the shipped one.
func liveEvals(t *testing.T) *langsmith.Evals {
	t.Helper()
	if os.Getenv(langsmith.DefaultKeyEnv) == "" {
		t.Skip("LANGSMITH_API_KEY unset; skipping live LangSmith test")
	}
	dataset := os.Getenv("KNO_LIVE_DATASET")
	if dataset == "" {
		t.Skip("KNO_LIVE_DATASET unset; skipping live LangSmith test")
	}
...
func TestLiveDatasetReads(t *testing.T) {
	...
	if counts.Total() == 0 {
		t.Fatal("the live dataset has no examples")
	}
	if counts.Holdout == 0 {
