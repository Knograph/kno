...
func TestRedact(t *testing.T) {
	t.Parallel()
	e := &Evals{key: "test-key-abc"}
	msg := "request failed with key test-key-abc and again test-key-abc"
	got := e.redact(msg)
	if strings.Contains(got, "test-key-abc") {
		t.Errorf("redact left the key in %q", got)
	}
	if !strings.Contains(got, "[redacted]") {
		t.Errorf("redact did not mark the secret in %q", got)
	}
...
func TestPageReaderHandlesAnyKeyOrder(t *testing.T) {
	t.Parallel()
	// next_cursor before items: the envelope walk must not assume order.
	body := `{"next_cursor":"cur-9","items":[{"id":"ex-1","inputs":{"question":"q"}}]}`
	...
	if !bytes.Contains(raw, []byte(`"ex-1"`)) {
		t.Errorf("row = %s, want the example object", raw)
	}
	if cursor, hasNext := pr.nextCursor(); !hasNext || cursor != "cur-9" {
		t.Errorf("nextCursor = %q,%v want cur-9,true", cursor, hasNext)
	}
...
func TestPageReaderIgnoresUnknownFields(t *testing.T) {
	t.Parallel()
	body := `{"items":[{"id":"ex-1","inputs":{"question":"q"}}],"something_new":{"x":1},"next_cursor":""}`
	pr, err := newPageReader(bytes.NewReader([]byte(body)))
	if err != nil {
		t.Fatalf("newPageReader: %v", err)
	}
...
func TestEndpointCheckCopiesTheDestinationPrecedent(t *testing.T) {
	...
	// These cases mirror adapters/agent/internal/transport/destination.go's
	// behavior; the adapter must not drift from the precedent it copied.
	tests := []struct {
		raw      string
		insecure bool
		private  bool
		...
		{raw: "https://api.smith.langchain.com", wantErr: ""},
		{raw: "http://api.smith.langchain.com", insecure: true, wantErr: ""},
...
func TestURLQueryBuiltWithEscaping(t *testing.T) {
	t.Parallel()
	u, err := url.Parse("https://example.com/examples")
	if err != nil {
		t.Fatal(err)
	}
	q := u.Query()
	q.Set("dataset_id", "a b&c")
	u.RawQuery = q.Encode()
	if got := u.String(); !strings.Contains(got, "dataset_id=a+b%26c") {
		t.Errorf("query not escaped: %s", got)
	}
}
...
func TestCheckResolvedAppliesThePolicyToTheResolvedAddress(t *testing.T) {
	...
	// These mirror the agent transport's checkResolved: the address here is
...
func TestRowCapBindsTheReadNotJustTheRow(t *testing.T) {
	...
	// drained — the old decoder-based reader buffered the whole row before
	// its size check, letting a hostile server push a page-sized allocation.
	big := `{"id":"ex-big","inputs":{"question":"` + strings.Repeat("q", (maxRowBytes*3)/2) + `"}}`
	body := `{"items":[` + big + `,{"id":"after"},{"id":"also-after"}],"next_cursor":""}`
