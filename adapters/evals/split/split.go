// Package split holds the dev/holdout division every Evals adapter shares.
//
// The denominator math must not vary by adapter: if jsonl and langsmith
// split differently, the population behind every confidence interval changes
// with the source, invisibly. One package, imported by both, makes
// "byte-for-byte" a compile-time fact instead of a copied hash that can
// drift.
package split
...
// Counts reports how an eval set divided.
type Counts struct {
...
func (c Counts) Total() int { return c.Dev + c.Holdout }

// Underpowered reports whether the holdout is too small to support a
// meaningful interval.
func (c Counts) Underpowered() bool { return c.Holdout > 0 && c.Holdout < MinHoldout }
