...
// dataset is absent. Host defaults to the hosted API; a self-hosted
// deployment is selected by BRAINTRUST_API_BASE_URL and must be opted into
// with the same flags the CLI uses, so the live posture matches the shipped
// one.
func liveEvals(t *testing.T) *braintrust.Evals {
	t.Helper()
	if os.Getenv(braintrust.KeyEnv) == "" {
		t.Skip("BRAINTRUST_API_KEY unset; skipping live Braintrust test")
	}
	dataset := os.Getenv("KNO_LIVE_DATASET")
	if dataset == "" {
		t.Skip("KNO_LIVE_DATASET unset; skipping live Braintrust test")
	}
...
func TestLiveDatasetReads(t *testing.T) {
	...
	if counts.Total() == 0 {
		t.Fatal("the live dataset has no events")
	}
	if counts.Holdout == 0 {
