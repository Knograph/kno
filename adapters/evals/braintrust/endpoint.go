...
// made.
//
// The API key travels to this host on every request — as a Bearer token —
// so the host itself must be one the key is allowed to reach. These rules
// are copied verbatim from adapters/evals/langfuse/endpoint.go, which
// copied them from adapters/evals/langsmith/endpoint.go, which itself
// copied them from adapters/agent/internal/transport's Destination check
// (destination.go) — the same policy the agent adapters enforce for the
// same reason. They are kept here, deliberately unshared, because that
// package is internal to adapters/agent and this adapter must not import
// it. If the three drift, this file is the one that needs reviewing.
func checkEndpoint(raw string, allowInsecureBaseURL, allowPrivateAddress bool) error {
	...
	if err != nil {
		...
		// it back, twice.
		return fmt.Errorf("braintrust: the endpoint could not be parsed")
	}
	...
	if u.User != nil {
		return fmt.Errorf("braintrust: the endpoint carries a credential in its userinfo; " +
			"remove it and set BRAINTRUST_API_KEY instead")
	}
	...
	case "http":
		if !allowInsecureBaseURL {
			return fmt.Errorf("braintrust: %s is plain HTTP, which sends the request and the "+
				"API key in the clear; set BRAINTRUST_API_BASE_URL to an https URL, or pass "+
				"--allow-insecure-base-url to accept the risk", u.Host)
		}
	default:
		return fmt.Errorf("braintrust: scheme %q is not http or https", u.Scheme)
	}

	if u.Host == "" {
		return fmt.Errorf("braintrust: the endpoint %q has no host", raw)
	}
	return checkAddress(u.Hostname(), allowPrivateAddress)
}

// checkAddress applies the private-address rules. Copied verbatim from
// adapters/evals/langfuse/endpoint.go (thence langsmith, thence
...
func checkAddress(hostname string, allowPrivateAddress bool) error {
	...
	if ip == nil {
		...
		if !plausibleDNSName(hostname) {
			return fmt.Errorf("braintrust: %q is neither a hostname nor a canonical IP "+
	...
	if ip.IsLinkLocalUnicast() || ip.IsLinkLocalMulticast() {
		return fmt.Errorf("braintrust: %s is link-local, which is where cloud instance "+
	...
	if ip.IsLoopback() || ip.IsPrivate() || ip.IsUnspecified() {
		return fmt.Errorf("braintrust: %s is a private address. A self-hosted Braintrust "+
...
// actually produced. Copied verbatim from
// adapters/evals/langfuse/endpoint.go, same rationale line for line.
//
...
// address in disguise. Copied verbatim from
// adapters/evals/langfuse/endpoint.go (thence langsmith, thence
// transport/destination.go).
//
