...
// bearerValue is the Authorization header value for Braintrust's documented
// auth: the API key as a Bearer token.
...
// redirect but NOT a same-host one — and this credential is the API key,
// not a per-host header. Refusing all redirects — same host or not — is the
// only safe policy, and the datasets and fetch endpoints have no legitimate
// reason to redirect.
func refuseRedirect(req *http.Request, _ []*http.Request) error {
	return fmt.Errorf("braintrust: refusing a redirect to %s; the credentials must not follow one", req.URL.Host)
}
...
func (e *Evals) do(ctx context.Context, path string, query url.Values) (*http.Response, error) {
	...
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		...
		if err != nil {
			return nil, fmt.Errorf("braintrust: building the request: %s", e.redact(err.Error()))
		}
		...
		if err != nil {
			...
			// leak the credential.
			lastErr = fmt.Errorf("braintrust: request failed: %s", e.redact(err.Error()))
			continue
		}
		...
		if attempt == maxAttempts {
			lastErr = fmt.Errorf("braintrust: the dataset API kept answering 429 after %d "+
				"attempts; it is throttling this address", maxAttempts)
			continue
		}
		lastErr = fmt.Errorf("braintrust: the dataset API answered 429; retrying in %s", wait)
...
//
// The credential is the Bearer token and the key itself. Errors must never
// carry either — they would land in a log line, a trace, or a report.
// Applied at every error-construction point rather than at the package
// boundary, so nothing can slip through.
func (e *Evals) redact(msg string) string {
	if e.bearer != "" {
		msg = strings.ReplaceAll(msg, e.bearer, "[redacted]")
	}
	return strings.ReplaceAll(msg, e.key, "[redacted]")
}

// pageLimitError reports the pagination backstop once more than maxPages
// pages have been fetched. A server that mints a fresh cursor per page is
// caught here rather than spun on forever; a REPEATED cursor is caught
// earlier, by the seen-cursor check.
func pageLimitError(fetched int) error {
	if fetched > maxPages {
		return fmt.Errorf("braintrust: pagination exceeded %d pages; the dataset keeps "+
