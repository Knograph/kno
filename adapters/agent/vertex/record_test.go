...
//
// NOTE: rawpredict-ok is hand-authored, and this target does not write it —
// see that fixture's note.txt. This file exists so the adapter's recording
// surface is exercised like its siblings' even though the current fixture is
// not a recording.
...
// recordModel is what a recording would target.
//
...
const recordModel = "claude-sonnet-4-5"

// captureRoundTripper records the exchange while the request proceeds
// normally. The request body is re-spliced before the inner transport sees it
// (the bearer hook reads it), and the response body is re-spliced for the
// caller.
...
// TestRecord re-records a fixture the way the sibling adapters' recording
// target does. It refuses every route out: no env, no run; no
// KNO_LIVE_TESTS, no run; no cap, no run; test credentials, no run. The
// captured exchange is written to rawpredict-ok/ so a hand-authored fixture
// can be replaced by the real exchange when an operator has live access.
func TestRecord(t *testing.T) {
	...
	for _, env := range []string{"GOOGLE_APPLICATION_CREDENTIALS", "GOOGLE_CLOUD_REGION"} {
		if os.Getenv(env) == "" {
			t.Fatalf("%s is not set; recording needs live credentials", env)
		}
	}
	if os.Getenv("GOOGLE_APPLICATION_CREDENTIALS") == "/secrets/kno-test-sa.json" {
		t.Fatal("the test credential path is set; that is not a live credential")
	}
	...
	dir := filepath.Join("testdata", "fixtures", "rawpredict-ok")
