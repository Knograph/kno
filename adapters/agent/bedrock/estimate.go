...
func (a *Agent) Estimate(ctx context.Context, c *core.Case) (budget.Estimate, error) {
	...
	if c == nil {
		return budget.Estimate{}, fmt.Errorf("bedrock: nil case")
	}
	return a.estimate(c)
}
...
func pricingRefusal(model string, err error) error {
	if class := pricing.RegionClass(model); class != "" {
		return errs.ErrInvalidInput.WithFix(fmt.Sprintf(
			...
			Wrap(fmt.Errorf("bedrock: %s: %w", model, err))
	}
	return err
}
...
// prompt for the full reasoning. Converse bills the same four parts: system
// array, injected Asset, history, input.
