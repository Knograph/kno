...
// context.go for the full reasoning; this file mirrors it with Converse's
// system-array shape.
...
// one rate limiter — and the skew-retry budget. Both are deliberate: the
// paired measurement is one run, and the clock is one clock.
//
// POSITION: the Asset joins the system array, immediately after the configured
// system prompt and ahead of the Case's history and input — the byte-identical
// prefix every Case in an Asset's sample shares, which is what lets the
// provider's cache price it at the far cheaper cached rate.
...
func (a *Agent) injectable(asset *core.Asset) (string, error) {
	if asset == nil {
		return "", fmt.Errorf("%w: bedrock: there is no Asset to inject", errs.ErrInvalidInput)
	}
	if a.asset != "" {
		...
		return "", errs.ErrInvalidInput.
			...
			Wrap(fmt.Errorf("bedrock: this Agent already carries an injected Asset"))
	}
	...
	if int64(len(a.opts.System)+len(content)) > ceiling {
		return "", errs.ErrInvalidInput.WithFix(fmt.Sprintf(
			...
			Wrap(fmt.Errorf("bedrock: asset %s is %d bytes and the system prompt is "+
...
func assetContent(asset *core.Asset) ([]byte, error) {
	...
	if len(content) == 0 {
		...
		return nil, errs.ErrInvalidInput.
			...
			Wrap(fmt.Errorf("bedrock: asset %s is empty", asset.GetId()))
	}
	if !utf8.Valid(content) {
		...
		return nil, errs.ErrInvalidInput.
			...
			Wrap(fmt.Errorf("bedrock: asset %s is not valid UTF-8", asset.GetId()))
	}
	return content, nil
}
...
// rides as one system-array payload and it is the WHOLE set that must fit
// ahead of every Case.
//
// POSITION: like WithContext, the joined set joins the system array,
...
func (a *Agent) WithContextSet(assets []*core.Asset) (core.Agent, error) {
	if len(assets) == 0 {
		return nil, errs.ErrInvalidInput.
			...
			Wrap(fmt.Errorf("bedrock: no Assets to inject; an Agent carrying none " +
	...
	for i, asset := range assets {
		if asset == nil {
			return nil, fmt.Errorf("%w: bedrock: there is no Asset at index %d of the set to inject",
	...
	if int64(len(a.opts.System)+len(joined)) > ceiling {
		return nil, errs.ErrInvalidInput.WithFix(fmt.Sprintf(
			...
			Wrap(fmt.Errorf("bedrock: the %d-Asset Portfolio is %d bytes and the system "+
