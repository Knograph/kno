...
// reached through a retriever. Everything here is arranged around one
...
// prompt carries the whole Asset. WorstCase's own godoc records that failure
// already measured once, quoting $0.06 for a run whose real exposure was
// $12.00. A copy of *Agent cannot forget.
//
// The copy shares the transport, so the two arms share one connection pool and
// one rate limiter — a 429 earned by the treatment arm paces the control arm
// too, which is what keeps a paired measurement from being two runs.
//
// POSITION: the Asset joins the top-level system field, immediately after the
// configured system prompt and ahead of the Case's history and input. The
// position is what matters, not the field — providers cache on a PREFIX, and
...
// whole sample, which costOf prices at its own far cheaper rate. Putting it
// after the history would put varying bytes ahead of it and every Case would
// pay for it fresh.
...
func (a *Agent) injectable(asset *core.Asset) (string, error) {
	if asset == nil {
		return "", fmt.Errorf("%w: anthropic: there is no Asset to inject", errs.ErrInvalidInput)
	}
	if a.asset != "" {
		...
		return "", errs.ErrInvalidInput.
			...
			Wrap(fmt.Errorf("anthropic: this Agent already carries an injected Asset"))
	}
	...
	// MaxPromptBytes is the only statement a caller makes about how large a
	// prompt this Agent may plan for. Unlike openaicompat, this adapter does
	// not enforce it per Case — its godoc says so — so it is checked at the one
	// moment the adapter holds the dominant term in its hand and the check
	// costs nothing. An Asset larger than the whole ceiling would otherwise be
	// priced into every reservation with no bound above it at all.
	...
	if int64(len(a.opts.System)+len(content)) > ceiling {
		return "", errs.ErrInvalidInput.WithFix(fmt.Sprintf(
			...
			Wrap(fmt.Errorf("anthropic: asset %s is %d bytes and the system prompt is "+
...
func assetContent(asset *core.Asset) ([]byte, error) {
	...
	if len(content) == 0 {
		...
		return nil, errs.ErrInvalidInput.
			...
			Wrap(fmt.Errorf("anthropic: asset %s is empty", asset.GetId()))
	}
	if !utf8.Valid(content) {
		...
		return nil, errs.ErrInvalidInput.
			...
			Wrap(fmt.Errorf("anthropic: asset %s is not valid UTF-8", asset.GetId()))
	}
	return content, nil
}
...
// rides as one system-field payload and it is the WHOLE set that must fit
// ahead of every Case.
//
// POSITION: like WithContext, the joined set joins the top-level system
// field, immediately after the configured system prompt and ahead of the
...
func (a *Agent) WithContextSet(assets []*core.Asset) (core.Agent, error) {
	if len(assets) == 0 {
		return nil, errs.ErrInvalidInput.
			...
			Wrap(fmt.Errorf("anthropic: no Assets to inject; an Agent carrying none " +
	...
	for i, asset := range assets {
		if asset == nil {
			return nil, fmt.Errorf("%w: anthropic: there is no Asset at index %d of the set to inject",
	...
	if int64(len(a.opts.System)+len(joined)) > ceiling {
		return nil, errs.ErrInvalidInput.WithFix(fmt.Sprintf(
			...
			Wrap(fmt.Errorf("anthropic: the %d-Asset Portfolio is %d bytes and the system "+
...
// is a reservation for a prompt other than the one that went out — invisible in
// review and invisible in the numbers.
