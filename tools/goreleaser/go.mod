// Module tools/goreleaser isolates the release tool from BOTH the shipping
// module and the tools module.
//
// It is not in tools/go.mod, and that is the point. goreleaser's graph is ~500
// packages — gocloud.dev, sigs.k8s.io/kind, k8s.io/klog, pkcs12 — and adding it
// there MVS-bumped grpc, genproto, yaml and x/time underneath golangci-lint and
// buf. A release tool must not be able to change what the lint gate compiles.
//
// Its own module means it is still byte-pinned by a committed go.sum, which
// matters more here than anywhere else in the repo: this binary runs in the one
// job holding `id-token: write`, where it can sign artifacts as this repository
// into a public transparency log. "Pinned by version and the checksum database"
// is not the guarantee that job deserves.
//
// Version ceiling: goreleaser v2.14+ requires Go >= 1.26, and the Makefile pins
// GOTOOLCHAIN to go.mod's toolchain directive. Bumping goreleaser past v2.13.x
// means bumping the toolchain first, in its own PR.
module github.com/knograph/kno/tools/goreleaser
