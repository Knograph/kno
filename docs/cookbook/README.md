...
`check-your-evals` is the one recipe still held here in full, and deliberately: it documents `kno
eval inspect`, which is on `main` and in no release. `kno-examples` checks every command on every
page against the binary you can download, so a page about an unreleased command can carry no
honest tier there. It migrates in the release window that ships the command.
