...
Goal: keep the numbers, drop the transcripts.
...
If your evals are built from production logs, the first two rows are end-user data sitting on whatever machine ran the command. Kno does not send it anywhere — there is no telemetry of content, ever — but it does keep it, because a measurement you cannot audit is a measurement you have to take on faith.
...
It tells you what it would remove and stops. Add `--yes` to go through with it:
...
```bash
# Yesterday's nightly baseline: keep the score, drop the transcripts
kno purge --run-id "nightly-$(date -I -d yesterday)" --yes
```

Purging a run that does not exist is an error rather than a silent success, so a typo in the run ID fails the job instead of quietly keeping data you meant to remove.
