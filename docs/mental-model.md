...
**Redundant is measured, not read.** Two Assets are redundant only when their measured per-Case effects are both statistically equivalent (an equivalence test against a margin the sample itself derives, never invented) *and* co-located on the same Cases (a chance-corrected overlap, not a raw one) — kind-agnostic, compared within the destination they compete for. That is an inference from two independent measurements, never a claim that the second Asset buys nothing jointly; a pair the tool cannot measure a shared slice for is `UNKNOWN`, and `UNKNOWN` selects both, because a missed duplicate costs recoverable budget while a false one silently deletes a real gain. Content similarity — the shipped 3-gram shingle rule — still decides when no measurement evidence exists, unchanged, for `KIND_KNOWLEDGE` Assets. `kno select --explain <asset-id>` prints the per-Case table a verdict rests on, free and read-only. See [what the numbers mean](what-the-numbers-mean.md#what-a-redundancy-claim-claims).
