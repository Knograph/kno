...
Per-Case attribution for orphan spend. The durable record needs the **money**; the attribution
already lives on the event stream, where `SettlementOvershoot` and `CaseErrored` name their Case.
That is what the spine is for.
